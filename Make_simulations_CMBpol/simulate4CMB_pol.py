...
def make_parallel_simu4CMB(in_parfile, out_file):
    ...
    write2h5(total_map_list_Q, label_list_Q, out_file + '_Q.h5')
    write2h5(total_map_list_U, label_list_U, out_file + '_U.h5')
...
def MakeSimu(pos, nm_parfile):
    ...
    from corrsky_v0124 import corrsky_TQU, fluxassoc_TQU
    import astropy
    ...
    Rbk_map, Rbk_mapp, R_bkcat, Rbk_catp = corrsky_TQU(nu = Simulation_Parameters['freq'],
    ...
    IRbk_map, IRbk_mapp,IR_bkcat, IRbk_catp = corrsky_TQU(nu = Simulation_Parameters['freq'],
                                                          npix = Simulation_Parameters['npix'],
                                                          pixsize = Simulation_Parameters['pixsize'],
                                                          sncmodel = 'lapi11', pkmodel = 'lapi11',
                                                          Sn = -4.5, Sx = min(Simulation_Parameters['Sx'],0.0),
                                                          Slim = None, zbin = None,
    ...
    IRLTbk_map, IRLTbk_mapp, IRLT_bkcat, IRLTbk_catp = corrsky_TQU(nu = Simulation_Parameters['freq'],
                                                                   npix = Simulation_Parameters['npix'],
                                                                   pixsize = Simulation_Parameters['pixsize'],
    ...
    for comp in Simulation_Parameters["contaminants"]:
        ...
        if comp == "cmb":
            label_Q = omap_forse
            #plt.figure(1)
            #plt.imshow(omap_forse), plt.colorbar()
            #plt.savefig('Label_143_Q.pdf')
            #plt.close()
    ...
    sigma_noise = Simulation_Parameters["spp"] * 1e-6 * (60. * 60. / beam_size)       
    ...
    # print('mean', mean(Umap), 'std', std(Umap))
    # print('mean', mean(patch_U), 'std', std(patch_U))
    # plt.figure(1)
    # plt.imshow(total_map_Q), plt.colorbar()
    # plt.savefig('Map_Q_total_143.pdf')
    
    return total_map_Q_low, total_map_U_low, total_map_Q, total_map_U, total_map_Q_high, total_map_U_high, label_Q, label_U
...
def main():
    ...
    make_parallel_simu4CMB('./sim_cfreq143.par', './simu_cfreq143/Train')
    #apply_smoothing('simu_cfreq143/Validation_Q_raw.h5', 'simu_cfreq143/Validation_Q.h5', [9.66, 7.22, 4.9], 30.,90.)
    #apply_smoothing('simu_cfreq143/Validation_U_raw.h5', 'simu_cfreq143/Validation_U.h5', [9.66, 7.22, 4.9], 30.,90.)
