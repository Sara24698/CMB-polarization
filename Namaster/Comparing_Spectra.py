...
data_Q = ['./Outputs_CENN_Q_suavizado.h5']
data_U = ['./Outputs_CENN_U_suavizado.h5']

color_Input = ['tab:green']
color_Output = ['tab:red']
color_Error = ['black', 'grey', 'purple']
color_Residuals = ['black']

label_Input = ['Input smoothed 30 arcmin']
label_Output = ['Output']
label_Error = ['r = 0.2', 'r = 0.1', 'r = 0.05']
...
class Read():
    ...
    def reading_noise_patches():
        ...
        data_Q = ['./Validation_Q_noise.h5']
        data_U = ['./Validation_U_noise.h5']
...
class Estimate_E_B_Power_Spectrum():
    ...
    def estimating_theoretical_power_spectrum_all_sky(Archivo_fits):
        ...
        Cls = hp.sphtfunc.anafast(CMB, lmax = 1000)
...
class Plot_E_B_Power_Spectrum():
    ...
    def plotting_E_B_Power_Spectrum_from_CENN():
        ...
        l_th_all_sky_Planck, EE_th_all_sky_Planck, BB_th_all_sky_Planck = Estimate_E_B_Power_Spectrum.estimating_theoretical_power_spectrum_all_sky('./CMB_143GHZ_SimPlanck_suavizado.fits')
        l_th_all_sky_0_2, EE_th_all_sky_0_2, BB_th_all_sky_0_2 = Estimate_E_B_Power_Spectrum.estimating_theoretical_power_spectrum_all_sky('./Mapa_total_143_GHz_R_0_2_suavizado.fits')
        ...
        for i in range(len(data_Q)):
            ...
            if i ==0:
                a0.plot(l_th_all_sky_Planck, EE_th_all_sky_Planck, 'black', label='EE Planck')
                a0.plot(l[i], ((EE[i][0]*(l[i]*(l[i]+1))/(2*np.pi))), label=label_Input[i])
                #a0.plot(l_noise[i], ((EE_noise[i][0]*(l_noise[i]*(l_noise[i]+1))/(2*np.pi))), color='grey', label='Noise')
                ...
                a1.plot(l[i], (((EE[i][1]-EE[i][0])*(l[i]*(l[i]+1))/(2*np.pi))), color=color_Error[i], label = label_Error[i])
                a1.fill_between(l[i], (((EE[i][1]-EE[i][0])*(l[i]*(l[i]+1))/(2*np.pi))) - ((
                    EE_Uncertainty[i][1]*(l[i]*(l[i]+1))/(2*np.pi))), (((EE[i][1]-EE[i][0])*(l[i]*(
                        l[i]+1))/(2*np.pi))) + ((EE_Uncertainty[i][1]*(l[i]*(l[i]+1))/(
            ...
            a0.set_xlim(10, 2001)
            a1.set_xlim(10, 2001)
            a0.set_ylim([0.001,200])
            a1.set_ylim([-15,35])
        
        
        plt.savefig('EE_comparison.pdf')
        """ 
...
# BB
        ...
        a0.plot(l_th_all_sky_Planck, BB_th_all_sky_Planck, label='BB Planck smoothed 30 arcmin')
        a0.plot(l_th_all_sky_0_2, BB_th_all_sky_0_2, label='BB all sky smoothed 30 arcmin')
        ...
        for i in range(len(data_U)):
            if i ==0:
                a0.plot(l[i], ((BB[i][0]*(l[i]*(l[i]+1))/(2*np.pi))), label=label_Input[i])
                #a0.plot(l[i], ((BB_noise[i][0]*(l_noise[i]*(l_noise[i]+1))/(2*np.pi))), color='grey', label='Noise')
                ...
                a0.plot(l[i], ((BB[i][1]*(l[i]*(l[i]+1))/(2*np.pi))), label=label_Output[i])
                ...
                a1.plot(l[i], (((BB[i][1]-BB[i][0])*(l[i]*(l[i]+1))/(2*np.pi))), color=color_Error[i], label = label_Error[i])
                a1.fill_between(l[i], (((BB[i][1]-BB[i][0])*(l[i]*(l[i]+1))/(2*np.pi))) - ((
                    BB_Uncertainty[i][1]*(l[i]*(l[i]+1))/(2*np.pi))), (((BB[i][1]-BB[i][0])*(l[i]*(
                       l[i]+1))/(2*np.pi))) + ((BB_Uncertainty[i][1]*(l[i]*(l[i]+1))/(
                           2*np.pi))), color=color_Error[i], alpha=0.2)
            else: 
                a0.plot(l[i], ((BB[i][0]*(l[i]*(l[i]+1))/(2*np.pi))), label=label_Input[i])
