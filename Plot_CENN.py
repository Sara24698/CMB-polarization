...
def plots(Filtro, Num_imagenes, validation_file_path):
    ...
    if Filtro == 'Q':   
        for i in range(Num_imagenes):
            ...
            ax0.title.set_text('Input Total (Q)')
            ax1.title.set_text('True CMB (Q)')
            ax2.title.set_text('Output CMB (Q)')
            ax3.title.set_text('Residual CMB (Q)')
            ...
            plt.savefig('Validation_Q_'+str(i)+'.pdf')
        ...
        for i in range(Num_imagenes):
            ...
            ax0.title.set_text('Input Total (U)')    
            ax4.title.set_text('True CMB (U)')
            ax5.title.set_text('Output CMB (U)')
            ax6.title.set_text('Residual CMB (U)')
            ...
            plt.savefig('Validation_U_'+str(i)+'.pdf')
