...
def reading_validation_set(validation_file_path, Patch_Size):
    ...
    for i in range(len(inputs_validation)):
        ...
        max_validation.append(np.max(inputs_validation[i,:,:,1]))
        min_validation.append(np.min(inputs_validation[i,:,:,1]))
    ...
    max_validation = np.array(max_validation)
    min_validation = np.array(min_validation)


    return inputs_validation, labels_validation, max_validation, min_validation
