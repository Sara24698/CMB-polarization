...
from CENN_Train import WeightedMSE
...
keras.utils.get_custom_objects()['WeightedMSE'] = WeightedMSE

from CENN_Input import reading_the_data, reading_validation_set, reading_model, reading_validation_set_no_normalized
...
def save_data(results_denorm, labels_validation_no_normalized, name_output_file):
    ...
    with h5py.File(name_output_file, 'w') as f:
        ...
        f['sim'] = labels_validation_no_normalized

def error(y_pred, y_true):

    return K.mean(K.square(y_pred - y_true), axis=-1)

def quick_execute(Filtro, validation_file_path, Patch_Size, model, maximo, minimo, test_inputs, test_outputs, load_data_bool=False):
...
def execute(Patch_Size, Filtro, validation_file_path):
    
    # normalised validation dataset
    Model_file_path = './Models_'+Filtro+'/_Red_'+Filtro+'.h5'
    
    inputs_validation = reading_validation_set(validation_file_path, Patch_Size)[0]
    labels_validation = reading_validation_set(validation_file_path, Patch_Size)[1]
    maximo = reading_validation_set(validation_file_path, Patch_Size)[2]
    minimo = reading_validation_set(validation_file_path, Patch_Size)[3]
